...
export function ContentDynamic ({ titleSection }) {
  ...
  const [palceholder, setPlaceholder] = useState('Filter by')
  ...
  function handleClick (ev) {
    ...
    setPlaceholder(name.charAt(0).toUpperCase() + name.slice(1))
    if (name === 'all') {
      setCurrentContent(contentArray)
    ...
    const filteredContent = contentArray.filter(cont => cont.type === name && cont.category === titleSection)
    setCurrentContent(filteredContent)
    setShowFilter(!showFilter)
  }
    ...
    <section className='flex flex-col gap-6'>
      <nav className='flex flex-row text-primary text-xl font-medium gap-4 align-start'>
        ...
        <div className='flex flex-col text-primary text-sm md:text-xl font-medium gap-1 items-start relative'>
          ...
          >
            {palceholder} <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' width='30' height='30'><path fill='none' d='M0 0h24v24H0z' /><path fill='currentColor' d='M12 13.172l4.95-4.95 1.414 1.414L12 16 5.636 9.636 7.05 8.222z' /></svg>

          </button>
          <ul className={`${showFilter ? 'block' : 'hidden'} z-50 h-auto text-primary text-xl border border-[white] rounded-lg w-full p-2 min-w-[150px] absolute bg-[var(--background-color)] top-11`}>
            <li
              data-class='all'
              onClick={handleClick}
